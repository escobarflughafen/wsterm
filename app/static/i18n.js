...
const ZH = {
  ...
  'RULES': '规则', 'EVENTS': '事件', 'INCOME': '收益', 'DECISION AUDIT': '决策审计', 'TO DO & RULES': '待办与规则', 'SATELLITE': '卫星仓', 'WHAT-IF': '情景模拟', 'DATA & FETCH': '数据与抓取', 'IMPORT': '导入', 'HELP': '帮助',
  ...
  'dividends, interest, fees, tax, FX conversions': '股息、利息、费用、税项、外汇兑换',
  'cash drift and FX translation not attributable to a campaign': '无法归属到持仓周期的现金变动与外汇折算',
  'the same number PERF shows': '与业绩表现页显示的数字一致',
  'Campaigns are ranked by dollars but the column is a rate, so removing a large winner that earned less than the portfolio average can raise the return. The freed cash is not reinvested; it sits idle for the rest of the history.': '持仓周期按金额排序，但该列是收益率，因此移除一个金额很大、收益率却低于组合平均的盈利周期，反而可能让收益率上升。释放出的现金不会再投资，会在其余时间里闲置。',
  'LOADING DECISIONS': '正在加载决策', 'Loading decisions…': '正在加载决策…',
  'The per-decision rows are fetched separately so the other screens do not carry them.': '逐笔决策数据单独加载，以免其他页面一起承担这部分体积。',
  'REBUILD DATA TO GENERATE THE DECISION AUDIT': '请重建数据以生成决策审计',
  'no evidence yet': '尚无证据', 'no matured score yet': '尚无到期评分',

  'AVERAGE RELATIVE FACTOR': '平均相对因子', 'Each matured score is one factor: security divided by benchmark for buys, benchmark divided by security for sells. The panels show the running average of those factors, never their product — a 60-day window overlaps its neighbours, so multiplying would count one market move dozens of times.': '每个到期评分对应一个因子：买入为证券除以基准，卖出为基准除以证券。各面板显示这些因子的滚动平均，而不是它们的乘积 —— 60 日窗口与相邻窗口高度重叠，连乘会把同一次行情重复计算几十遍。',
  '10–90% BAND': '10–90% 区间', 'Campaigns are resampled as blocks and every decision inside a drawn campaign is pooled, so the band brackets the same average the table prints. This is a concentration-sensitivity range, not a statistical confidence interval and not a probability of skill.': '以完整持仓周期为区块重采样，抽中的周期内所有决策一并汇总，因此区间覆盖的正是表中打印的那个平均值。这是集中度敏感性区间，不是统计置信区间，也不是能力概率。',
  ...
  'Replay day': '回放日期', 'Anywhere on AUD: arrows move one day, Shift+arrows move five, Vim H/L move one': '在审计页任意位置：方向键移动一天，Shift+方向键移动五天，Vim H/L 移动一天',
  'ANYWHERE ON AUD: ←/→ = 1 DAY · SHIFT+←/→ = 5 · VIM H/L = 1 · SLIDER ALSO: h/l, HOME/END': '审计页全局：←/→ = 1 天 · Shift+←/→ = 5 天 · Vim H/L = 1 天 · 滑块另支持 h/l、Home/End',
  ...
  'No fixed-horizon score matured on this day; the averages carry forward.': '此日没有固定周期评分到期；平均值沿用前一日。',
  ...
  'The selected observation window has fully elapsed. Only now does its score enter the average.': '所选观察周期已经完整结束；评分到此刻才计入平均值。',
...
const ZH_PATTERNS = [
  ...
  [/^No decision was made and no (\d+)-trading-day score matured\. The averages carry forward unchanged; (\d+) earlier decisions remain pending\.$/, '今天没有新决策，也没有 $1 个交易日评分到期。平均值保持不变；此前仍有 $2 个决策待观察。'],
  [/^one panel per decision class · (\d+)-day scores averaged, never compounded · 1\.0000× is neutral$/, '每个决策类别一格 · $1 日评分取平均而非连乘 · 1.0000× 为中性'],
  [/^(\d+) campaigns, realized \+ unrealized$/, '$1 个持仓周期，已实现 + 未实现'],
  [/^(\d+) campaigns end or begin at a share transfer between Wealthsimple accounts\. A transfer moves shares at a stated book value rather than selling them, so P&L stops and restarts at that value instead of running through\.$/, '有 $1 个持仓周期在 Wealthsimple 账户间的股票转移处结束或开始。转移按账面价值搬运股票而不是卖出，因此盈亏在该价值处中断并重新起算，而不是连续计算。'],
  [/^WITHOUT TOP (\d+)$/, '移除前 $1'],
  ...
  [/^\$([\d,]+) left · (\d+) shares @ \$([\d,.]+) · on a dip$/, '剩 $$$1 · 可买 $2 股 @ $$$3 · 回调中'],
  [/^\$([\d,]+) left · (\d+) shares @ \$([\d,.]+) · no dip \(([-+][\d.]+)% 5D\)$/, '剩 $$$1 · 可买 $2 股 @ $$$3 · 无回调（五日 $4%）'],
  [/^sell ([\d.]+) of ([\d.]+) shares \(~\$([\d,]+)\) · now ([\d.]+)% of invested, cap (\d+)%$/, '卖出 $1 / $2 股（约 $$$3）· 现占投资部分 $4%，上限 $5%'],
  [/^expires (\d{4}-\d{2}-\d{2}) · (\d+)d · ([-+][\d,]+) (\w+) if it expired today$/, '$1 到期 · 还剩 $2 天 · 今天到期则 $3 $4'],
  [/^(\d{4}-\d{2}): (\d+) used, (\d+) left$/, '$1：已用 $2 笔，剩 $3 笔'],
  [/^last one (.+) · your trades run to (\d{4}-\d{2}-\d{2}) · (\d+) lines differ$/, '上次 $1 · 交易已到 $2 · $3 行不一致'],
  // 规则明细
  [/^\$([\d,]+) of \$([\d,]+) · \$([\d,-]+) left$/, '已用 $$$1 / 上限 $$$2 · 剩 $$$3'],
  [/^(\d{4}-\d{2}): \$([\d,-]+) of \$([\d,]+)$/, '$1：$$$2 / $$$3'],
  [/^(\w+) ([A-Z0-9.]+) \$([\d,]+) \(([\d.]+)% of invested\)$/, '$1 $2 $$$3（占投资部分 $4%）'],
